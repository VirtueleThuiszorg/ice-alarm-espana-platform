...
| CC6 | Away status and the structured Spanish address | ⬜ MISSING | `members.away_from` / `away_until` / `pendant_with_member` and `urbanizacion` / `bloque` / `portal` / `escalera` are applied and member-writable by design, and nothing renders them yet. Next increment. |
