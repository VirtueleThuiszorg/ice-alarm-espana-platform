...
> **`main`'s drift gate is RED until these five are pushed, and that is the alarm working.**
...
| S18 | **`supabase db push`, then append five filenames to `APPLIED_TO_PROD.txt`** | laptop | There is **no migrate workflow** — `deploy-functions.yml` does functions only. **FIVE** migrations are on main and not in production, and they apply in this order: `20260909100000_sales_command_stats_fix.sql`, `20260909110000_payment_link_order.sql`, `20260909120000_rota_2026_seed_and_generator.sql`, `20260909121500_notify_staff.sql`, `20260909130000_lead_new_router_emit.sql`. The push takes all five — **record all five**, because a push that applies five and records three leaves the manifest lying in the direction the drift gate cannot see. **Before it, check the four operators exist**, or the rota seed is a no-op and tells you so: `select email from staff where email in ('asoares@icealarm.es','cnicolas@icealarm.es','mbonner@icealarm.es','travis@icealarm.es');` — expect 4 rows. If fewer than 4 but more than 0, the seed **refuses** and names the missing one rather than half-filling the rota. If 0, it skips; create them and run `select public.seed_rota_2026();`. Then confirm: `select count(*) from staff_shifts where shift_date between '2026-09-10' and '2026-12-31';` — expect **339** | ⬜ |
