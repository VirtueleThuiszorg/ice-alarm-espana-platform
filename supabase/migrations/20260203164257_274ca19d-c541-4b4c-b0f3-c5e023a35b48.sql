...
('member-welcome', 'Welcome Email', 'Sent after successful registration and payment', 'member', 
  ...
  '<h1>Welcome to Care Conneqt!</h1><p>Hello {{first_name}},</p><p>Thank you for joining Care Conneqt. Your membership is now active.</p><p>Order: {{order_number}}<br>Amount: €{{amount}}</p><p><a href="{{dashboard_url}}">Access Your Dashboard</a></p><p>Best regards,<br>Care Conneqt Team</p>',
  '<h1>¡Bienvenido a Care Conneqt!</h1><p>Hola {{first_name}},</p><p>Gracias por unirse a Care Conneqt. Su membresía está activa.</p><p>Pedido: {{order_number}}<br>Importe: €{{amount}}</p><p><a href="{{dashboard_url}}">Acceder a Mi Panel</a></p><p>Saludos cordiales,<br>Equipo Care Conneqt</p>',
  '["first_name", "order_number", "amount", "dashboard_url"]'::jsonb
),
('member-verification', 'Email Verification', 'Email verification link', 'member',
  'Verify your email address - Care Conneqt',
  'Verifique su correo electrónico - Care Conneqt',
...
('member-password-reset', 'Password Reset', 'Password reset request email', 'member',
  'Reset your password - Care Conneqt',
  'Restablecer su contraseña - Care Conneqt',
...
('support-ticket-created', 'Support Ticket Created', 'Confirmation when support ticket is created', 'support',
  'Support Ticket #{{ticket_number}} Created - Care Conneqt',
  'Ticket de Soporte #{{ticket_number}} Creado - Care Conneqt',
...
('outreach-intro', 'Outreach Introduction', 'Initial outreach email to prospects', 'outreach',
  'Protect Your Loved Ones with Care Conneqt',
  'Proteja a sus Seres Queridos con Care Conneqt',
  '<h1>Peace of Mind for You and Your Family</h1><p>Hello {{first_name}},</p><p>Are you or a loved one living independently in Spain? Care Conneqt provides 24/7 emergency response with just the press of a button.</p><p><a href="{{landing_url}}">Learn More</a></p>',
  '<h1>Tranquilidad para Usted y su Familia</h1><p>Hola {{first_name}},</p><p>¿Usted o un ser querido vive de forma independiente en España? Care Conneqt ofrece respuesta de emergencia 24/7 con solo presionar un botón.</p><p><a href="{{landing_url}}">Más Información</a></p>',
...
('outreach-followup-1', 'Outreach Follow-up 1', 'First follow-up email', 'outreach',
  ...
  '<h1>We are Here to Help</h1><p>Hello {{first_name}},</p><p>We wanted to follow up on our previous message about Care Conneqt. Do you have any questions about how our service works?</p><p><a href="{{landing_url}}">Get Protected Today</a></p>',
  '<h1>Estamos Aquí para Ayudar</h1><p>Hola {{first_name}},</p><p>Queríamos hacer seguimiento a nuestro mensaje anterior sobre Care Conneqt. ¿Tiene alguna pregunta sobre cómo funciona nuestro servicio?</p><p><a href="{{landing_url}}">Protéjase Hoy</a></p>',
  '["first_name", "landing_url"]'::jsonb
),
('partner-welcome', 'Partner Welcome', 'Welcome email for new partners', 'system',
  ...
  'Bienvenido al Programa de Socios Care Conneqt - Sus Credenciales',
...
('staff-welcome', 'Staff Welcome', 'Welcome email for new staff members', 'system',
  'Your Care Conneqt Staff Account Has Been Created',
  'Tu cuenta de personal de Care Conneqt ha sido creada',
  '<h1>Welcome to Care Conneqt Staff Portal</h1><p>Hello {{first_name}},</p><p>Your staff account has been created with the role: <strong>{{role}}</strong></p><p><strong>Email:</strong> {{email}}<br><strong>Temporary Password:</strong> {{temp_password}}</p><p><a href="{{login_url}}">Login to Staff Portal</a></p><p>Please log in and change your password immediately.</p>',
  '<h1>Bienvenido al Portal de Personal Care Conneqt</h1><p>Hola {{first_name}},</p><p>Tu cuenta de personal ha sido creada con el rol: <strong>{{role}}</strong></p><p><strong>Email:</strong> {{email}}<br><strong>Contraseña temporal:</strong> {{temp_password}}</p><p><a href="{{login_url}}">Iniciar Sesión</a></p><p>Por favor, inicia sesión y cambia tu contraseña inmediatamente.</p>',
