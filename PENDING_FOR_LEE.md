...
| S39 | **Run one Stripe test-clock rehearsal before switching the runner on** | Stripe dashboard → test mode → Customers → test clocks | **Nothing in the migration has been run against Stripe: this environment has no Stripe key, so no session was ever created and no clock was ever advanced.** Everything shipped is proven by unit tests, contract tests and 664 RLS assertions against real PostgreSQL — which covers the decisions and the database, and cannot cover Stripe's behaviour. The rehearsal worth doing, in test mode: take a monthly member through a switch link on the 15th, check the FULL fee is taken at once (not €0, not prorated), advance the clock a month and check the next charge lands on the 15th, and check `billing_source` flipped to `stripe` on the first payment and that the Santander export no longer lists them. Then the same again paying by SEPA, to see `async_payment_succeeded` arrive. The runner is OFF until you turn it on (Admin → Settings → Billing), and its "Preview today's list" shows exactly who would be written to before you do  **This is now one command rather than a checklist**: `STRIPE_TEST_KEY=sk_test_... node scripts/stripe/test-clock-rehearsal.mjs --amount <cents>` creates a test clock, a customer and a subscription with the switch link's exact parameters, and asserts each outcome — pass or fail per step, non-zero exit. It REFUSES without a key rather than skipping, and refuses a live key outright. Worth a second run with `--day 31`, the month-end clamp. It covers all four Stripe-side outcomes — card first charge, card renewal, a SEPA debit that settles, and one that bounces with a single retry scheduled. Its judgement is executed (31 tests, nine mutations); its HTTP has never run against the live API, so the first run may need a field name corrected — every read reports the missing field by name rather than crashing. Completing one real Checkout Session still needs a browser and stays manual | ⬜ |
