...
function isEventType(value: unknown): value is NotifyEventType {
...
serve(async (req) => {
  ...
  try {
    ...
    if (!body || typeof body !== "object") return json(400, { error: "A JSON body is required" });
    ...
    if (!event.title?.trim() || !event.body?.trim()) {
      return json(400, { error: "event.title and event.body are required" });
    }
    if (!audience || ((audience.roles ?? []).length === 0 && (audience.staffIds ?? []).length === 0)) {
      return json(400, {
