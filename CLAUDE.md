...
**⚠️ As of 11 Sep 2026 16:28 UTC the ruleset is ACTIVE but still does not do that**, so the line above is the rule and not yet the state of the repository. Ruleset `main` (id 19055263) now has `enforcement: active` and targets `~DEFAULT_BRANCH` — but it carries **no `required_status_checks` rule**, so a red check blocks nothing; it blocks merge commits (`allowed_merge_methods` omits `merge`); and it has **no bypass actors**, which is why `migrate.yml` can no longer record what it applied. Verified against the live API and proven by a throwaway PR that GitHub reported as `mergeable_state: "unstable"` with `Tests` red. The response JSON, the three deviations and what is still needed are in `PENDING_FOR_LEE.md` §2 S16; `Merge gate is enforced` in `ci.yml` re-checks it on every push and is red until it is fixed. **Until then, "never merge red" is enforced by discipline alone, and that has now failed five times: 23 July, 25 July, and three PRs on 11 September — every time with the guard test already red on the PR.**
