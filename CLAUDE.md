...
**Merges to main are gated by the repository ruleset, and a red required check cannot be merged past by anyone** — no human, no session, no bot, and no repository admin; the gate is the record of "never merge red", not anybody's memory of it. The six required checks are Tests, Lint/Type Check & Build, Wiring register, Cross-tenant isolation, Security Audit and Migration drift gate.

That sentence became true on **12 Sep 2026**, and the caveat that stood here for a day is gone with it. Ruleset `main` (id 19055263) is `enforcement: active` on `~DEFAULT_BRANCH` and carries a `required_status_checks` rule naming all six, each pinned to `integration_id: 15368` so only a check produced by GitHub Actions can satisfy it. `Merge gate is enforced` in `ci.yml` re-reads `GET /rules/branches/main` on every push and goes red the moment that stops being so — it is not a one-off verification but a standing one. The response JSON is recorded in `PENDING_FOR_LEE.md` §2 S16.

Before that date "never merge red" was discipline alone, and it failed five times — 23 July, 25 July, and three PRs on 11 September, every one with the guard test already red on the PR. That is what the ruleset replaces. Two consequences worth keeping in mind: `Manifest matches production` is deliberately **not** required (it is push-only and reports `skipped` on every PR, so requiring it would leave every pull request pending for ever), and the `pull_request` rule refuses every direct push to main — so anything in CI that writes to main must authenticate with the `MANIFEST_PUSH_TOKEN` PAT, which `RULE 6` in `src/test/ciJobIsolation.test.ts` enforces across every workflow.
