...
serve(async (req) => {
  ...
  try {
    ...
    if (!auth.ok) {
      console.warn(`[ev07b-sos-alert] auth rejected: ${auth.reason}`);
      return new Response(
    ...
    const body: SosAlertPayload = JSON.parse(rawBody);
    ...
    // HIGHEST PRIORITY: Notify emergency contacts first — and read the answer.
    await notifyEmergencyContacts(baseUrl, serviceKey, {
