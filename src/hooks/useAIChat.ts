...
export function useAIChat(options: UseAIChatOptions | string = {}) {
  ...
  const createWelcomeMessage = useCallback((): ChatMessage => {
    ...
    if (userRole === "admin" && staffName) {
      ...
      if (i18n.language === "es") {
        content = `${greeting}, ${staffName}! 👋 Soy el Main Brain, tu asistente de IA para la gestión del sistema ICE Alarm España. Puedo ayudarte con análisis de datos, gestión de miembros, reportes, y coordinar las operaciones del negocio. ¿En qué puedo ayudarte hoy?`;
      } else {
        content = `${greeting}, ${staffName}! 👋 I'm the Main Brain, your AI assistant for managing the ICE Alarm España system. I can help you with data analysis, member management, reports, and coordinating business operations. What can I help you with today?`;
      }
