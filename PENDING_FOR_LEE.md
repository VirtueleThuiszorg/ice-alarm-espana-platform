...
| S37 | 🔴 **Enable SEPA Direct Debit and subscribe the webhook destination to the two async events, then tick the acknowledgement** | Stripe dashboard → Payments + Developers → Webhooks, then Admin → Settings → Payments | **This is the one thing that decides whether the billing migration runs or needs 431 phone calls.** Every one of these members has paid by direct debit for years; a switch link that offers only a card asks a 79-year-old to find one. But a SEPA checkout completes `unpaid` and activates only on `checkout.session.async_payment_succeeded` — so if the destination is not subscribed to it, the member signs the mandate, the money leaves days later, and **nothing activates them** while `switch_pending` has already taken them out of the Santander run. They would be paying twice as much attention to us and billed by nobody. The platform refuses to offer SEPA until you tick the acknowledgement, so today every switch link is card-only. Events needed on the destination: `checkout.session.async_payment_succeeded` **and** `checkout.session.async_payment_failed` — **both now do something**: the first activates them, the second returns them to Santander billing with a staff bell, so a bounced first debit is collected the old way instead of leaving them in neither collection for a fortnight. Until 2026-09-11 the settings card named `checkout.session.failed`, which is not a Stripe event; subscribing to what it said would have enabled nothing | ⬜ |
| S40 | **WhatsApp for the switch link: switch the channel on, and get a message template approved** | Admin → Settings → Notifications (`notify_channel_whatsapp`), Twilio → WhatsApp sender, then Meta for the template | Lee's brief names the delivery as "SMS/WhatsApp". The code now offers both on a switch link and reports each separately — but **the channel is off and no template exists**, so today every switch link goes by SMS and email only, and the WhatsApp row reads "not sent — the channel is switched off". The reason it is worth doing: these members are in their seventies and eighties, many read WhatsApp and ignore a text from a number they do not recognise, and a text about money from an unrecognised number is exactly what a scam looks like. **A business-initiated WhatsApp message outside a 24-hour conversation window needs an APPROVED TEMPLATE at Meta** — that is the long pole, not the switch. The template has to carry the member's first name, the amount, the cycle and the link | ⬜ |
| S38 | **Confirm the seven Stripe Prices exist in LIVE mode, not only test mode** | Stripe dashboard → Products, then Admin → Settings → Pricing → "Sync prices to Stripe" | Every switch link names a synced Price id — the browser sends no amounts, by construction. `resolveCheckoutLines` refuses rather than guessing when a Price is missing or stale, so in live mode a missing Price is a refusal with the key named, not a wrong charge. Worth clearing before the first bulk run rather than discovering it on 431 members | ⬜ |
| S39 | 🔴 **Put a Stripe TEST key where the rehearsal job can read it** — this is the single thing blocking the whole Stripe-side proof | GitHub → Settings → Secrets and variables → **Actions** → *New repository secret*, name `STRIPE_TEST_KEY`, value `sk_test_…` | **Still nothing in the migration has been run against Stripe. Twelve dispatched runs of `stripe-rehearsal.yml` have all logged `env: STRIPE_TEST_KEY:` EMPTY and stopped at the guard, so no session was ever created and no clock was ever advanced.** The rehearsal no longer needs a laptop — it runs in CI precisely so the key never leaves GitHub (`workflow_dispatch` only; GitHub → Actions → *Stripe rehearsal* → Run workflow, with `amount` in cents and `day` = a member's Santander day). **What has been eliminated, so you are not hunting where I already looked:** 8 candidate names × 2 stores — secrets *and* variables — is 16 combinations, every one `not set`, so it is neither a misspelling nor an Actions *variable*; and the API paths that would settle it from outside (`/actions/secrets`, `/actions/organization-secrets`, `/actions/variables`, `/environments`) are all blocked to me. **What is left is three things only you can see:** a **repository** secret (the box above — the normal answer); an **organisation** secret whose *Repository access* list omits this repo; or an **environment** secret — and that last one no longer needs a pull request, because the workflow now takes the environment's **name as a dispatch input** (leave it blank for the other two). Worth one glance elsewhere: `.env.example` records this platform's Stripe keys living in Supabase `system_settings`, which would make "it is set" true of the wrong place. **Not the live key — see S41.** What is already proven without Stripe: ~4,900 unit and contract tests and 680 RLS assertions against real PostgreSQL, which covers our decisions and our database and cannot cover Stripe's behaviour. The rehearsal covers the rest — card first charge at the FULL fee (not €0, not prorated), card renewal on the same day, the month-end clamp on `--day 31`, a SEPA debit that settles after the clock advances, and one that bounces with exactly one retry scheduled — printing the Stripe object ids it relied on and naming which of your rules any failure breaks. Its judgement is executed (46 tests, nine mutations); its HTTP has never run, so the first real run may need a field name corrected — every read reports the missing field by name rather than crashing. The runner stays OFF until you turn it on (Admin → Settings → Billing). Completing one real Checkout Session still needs a browser and stays manual | ⬜ |
| S43 | **Count the incomplete contact-form leads, then validate the constraint** — one query, then one line | Supabase SQL editor | `leads_contact_form_is_actionable` (migration `20260917130000`) is deliberately **`NOT VALID`**: it governs every new row and has **not** scanned the ones already there. Nobody working on this has production access, so the number of existing `source = 'contact_form'` rows with a blank name, email or phone is **unknown** — stated rather than guessed. The counting query is written into the migration and repeated here:<br><br>`select count(*) filter (where first_name is null or length(trim(first_name)) = 0) as no_name, count(*) filter (where email is null or length(trim(email)) = 0) as no_email, count(*) filter (where phone is null or length(trim(phone)) = 0) as no_phone, count(*) as total from public.leads where source = 'contact_form';`<br><br>**All zeroes** → run `alter table public.leads validate constraint leads_contact_form_is_actionable;` and the rule covers the history too. **Not zeroes** → those rows are still readable, still in the list and still belong to whoever sent them; decide what they are before making the database refuse their shape. Nothing is blocked either way — the door is already shut on new ones (`public-submit` refuses them, and the anon INSERT policy on `leads` is revoked), so this is closing the books, not a fix | ⬜ |
| S44 | **Turn the channels on so a staff member's introduction actually leaves the building** — and get ONE more WhatsApp template approved | Admin → Settings → Notifications (`notify_channel_sms`, `notify_channel_whatsapp`), then Meta for the template | Staff can now add a lead by hand and introduce ICE Alarm to them by email, SMS or WhatsApp from editable templates, with a personal join link. **Today every one of those sends reports `skipped: channel off`.** That is not a failure and the screen says so honestly — the link is always on screen and an operator reads it out or pastes it themselves, which for now is how it reaches anybody — but it is a feature working at a fraction of its value. Email needs nothing beyond §8 being finished. SMS needs only the switch. **WhatsApp needs a template approved at Meta, and it is a DIFFERENT template from S40's** — that one carries an amount and a billing cycle for the switch link; this one carries a first name, a staff member's name, a join link and the 24-hour number (`lead.intro_whatsapp`, editable at Admin → Settings → Notifications → "What we say to a lead"). The wording is seeded in all three languages and measured at 201–224 of 320 characters rendered, so it fits two GSM segments with a real link. **Worth doing in this order:** SMS switch first (one click, immediate value), then the Meta template. Nothing is blocked meanwhile | ⬜ |
| S45 | **Check the follow-up reminder actually fires, once** | Supabase → Database → Cron jobs, then the bell on any operator's screen | `lead-followup-runner` is scheduled at 07:00 UTC daily and bells the operator who owns a lead nobody has written to for `lead_followup_days` (seeded 3, editable as a settings row). It is safe before anybody uses leads — with none open and assigned it reads zero rows and returns `reminded: 0` — so there is nothing to do now. **What to check once leads are being worked:** that the job appears in `cron.job`, that a lead left alone for three days produces exactly ONE bell, and that writing to that lead re-arms it for the next quiet spell. The runner refuses itself safely if the Vault `service_role_key` secret is missing (a WARNING, not an error spike) — the same prerequisite as every other cron here | ⬜ |
