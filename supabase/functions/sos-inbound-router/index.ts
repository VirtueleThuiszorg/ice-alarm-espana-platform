    ...
    if (action === "initial") {
      ...
      return twiml(
        `<Say language="en-GB" voice="Polly.Amy">` +
          `Your ${esc(relationship)}, ${esc(memberName)}, has an active emergency alert with ICE Alarm España. Press 1 to join the call.` +
        `</Say>` +
        `<Say language="es-ES" voice="Polly.Lucia">` +
          `Su ${esc(relationship)}, ${esc(memberName)}, tiene una alerta de emergencia activa con ICE Alarm España. Pulse 1 para unirse a la llamada.` +
        `</Say>` +
