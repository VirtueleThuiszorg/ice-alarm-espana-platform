...
| G7a | The conversation LIST still previews from `messages` | ⬜ KNOWN REMAINDER | An Isabella-only conversation now opens to a full transcript, but its row in the list has an empty preview, because `last_message_preview` reads `messages`. Same shape as M23 and the same fix — deliberately left to whichever of the two is done first rather than solved twice. |
