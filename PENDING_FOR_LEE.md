...
| **#260 — `[HOLD FOR LEE] schema: rota`** | The 2026 rota: `bank_holidays`, `staff_shift_swaps`, `generate_rota()`, `seed_rota_2026()` and the 339-shift import. Held because the goal said to hold it, not because anything is unproven — it applies cleanly to a real PostgreSQL 16 and the harness runs **471 checks green**, 35 of them new, with 8 mutations each shown red. Order: `supabase db push` → append to `APPLIED_TO_PROD.txt` → merge that. See **S18**; the seed is a documented no-op until the four operator staff rows exist |
