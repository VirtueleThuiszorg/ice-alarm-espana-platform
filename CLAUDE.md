...
## Stack
Single Vite + React 18 + TypeScript SPA (npm, **not** a pnpm monorepo) · Tailwind + shadcn/ui in `src/components/ui` · Supabase (Postgres/Auth/Edge Functions/Realtime), one project — **`crpsuhoixfdhjugprbuc`** (ice-alarm-espana-prod, LifeLink Sync org, Pro; LOCKED 2026-07-22, LAUNCH_SCOPE.md §0; renamed from care-conneqt-prod 2026-09-03 — the ref is what identifies it, the display name is cosmetic). The planned `cfwnrcogikjycjcobsay` migration is **CANCELLED** (historical); `qkfvojbcxaptufsepupo` is **DEFERRED** — a possible future migration target, never a deploy target, and **not** to be deleted (correction 2026-08-11). Note `qkfvojbcxaptufsepupo` is *also* ICE-named: identify production by **ref**, never by brand name. Per-file ref map: `PROJECT_REFS.md`. · Stripe + Mollie (SEPA + cards, webhook-driven) · AI is Isabella on the **Anthropic API** (`claude-opus-4-8` via `_shared/anthropic.ts`, `ISABELLA_MODEL` overridable; core migrated 2026-07-24 — only the archive-candidate growth fns still touch Lovable) · Vercel deploy · Sentry.
